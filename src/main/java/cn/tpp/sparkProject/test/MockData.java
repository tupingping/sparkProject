...
public class MockData {
	...
	public static void mock(JavaSparkContext jsc, SQLContext sc){
		
		List<Row> rows = new ArrayList<Row>();
		
		String[] searchKeys = new String[]{"火锅", "蛋糕", "重庆辣子鸡", "重庆小面","呷哺呷哺", "新辣道鱼火锅", "国贸大厦", "太古商场", "日本料理", "温泉"};
			...
			for (int j = 0; j < 10; j++) {
				...
				for (int k = 0; k < random.nextInt(100); k++) {
					...
					if("search".equals(action)) {
						searchKeyword = searchKeys[random.nextInt(10)];   
		...
		DataFrame df = sc.createDataFrame(rowsRDD, schema);
		...
		DataFrame df2 = sc.createDataFrame(rowsRDD, schema2);
		for(Row _row : df2.take(10)) {
			System.out.println(_row);  
		}
		...
		DataFrame df3 = sc.createDataFrame(rowsRDD, schema3);
		for(Row _row : df3.take(10)) {
			System.out.println(_row);  
		}
